...
void pent_driver(int in, int cn) {
    ...
    if (cn) { // player activated pent
        if (ch[cn].level > level * 2 + 60 + 120) {
            log_char(cn, LOG_SYSTEM, 0, "You cannot use this pentagram. You are too strong (max level %d).", level * 2 + 60 + 120);
            return;
        }
        if (!status) { // player use only allowed when pent is not active
			...
			int exp;
			exp = RANDOM(level_value(ch[cn].level));
			give_exp_bonus(cn, min(level_value(ch[cn].level)  , exp)); // Activating a Pent gives exp
			log_char(cn, LOG_SYSTEM, 0, "You got %d experience points!", exp); // Tell how much exp you got
    ...
    for (n = 0; n < maxspawn; n++) {
        ...
        if (!co || !ch[co].flags || (unsigned short)ch[co].serial != (unsigned short)ser) {
            sprintf(name, "penter%d", level * 2 + RANDOM(5)); // was RANDOM(2));
