...
int get_fight_skill(int cn) {
	...
	an = ch[cn].item[WN_LHAND];

    if (an < 0 || an >= MAXITEM) {
        elog("get_fight_skill(): character %s (%d) is wielding illegal item %d. Removing.", ch[cn].name, cn, an);
        ch[cn].item[WN_LHAND] = 0;
        an = 0;
    }

    if (!an || !(it[an].flags & IF_WEAPON)) { // not wielding anything, or not wielding a weapon
        return ch[cn].value[0][V_HAND];
    }
    ...
    if (it[an].flags & IF_HAND) n = max(n, ch[cn].value[0][V_HAND]);
    if (it[an].flags & IF_DAGGER) n = max(n, ch[cn].value[0][V_DAGGER]);
    if (it[an].flags & IF_STAFF) n = max(n, ch[cn].value[0][V_STAFF]);
